...
WPromise<int> FetchUserAge() {
  return [] -> Promise<int> { co_return 25; };
}

WPromise<std::string> FormatAgeMessage(int age) {
  return [age] -> Promise<std::string> {
    co_return "User age: " + std::to_string(age);
  };
}

WPromise<void> BasicChainExample() {
  return [] -> Promise<void> {
...
WPromise<void> ChainWithFunctions() {
  return [] -> Promise<void> {
...
WPromise<void> ErrorHandlingExample() {
  return [] -> Promise<void> {
