...
Promise<int, true> AsyncOperation(Resolve<int> const &resolve, Reject const &) {
...
Promise<int, true> ConditionalResolve(Resolve<int> const &resolve,
                                      Reject const &reject, bool success) {
  if (success) {
    resolve(100);
  } else {
...
Promise<void> ResolverStyleExample() {
  ...
  int value = co_await prom;
  std::cout << "Direct resolve result: " << value << "\n\n";

  co_return;
}

Promise<void> CreateHelperExample() {
...
Promise<void> ConditionalExample() {
