...
WPromise<void> CVPromiseNotifyExample() {
  return [] -> Promise<void> {
...
WPromise<void> CVPromiseResetExample() {
  return [] -> Promise<void> {
    ...
    std::thread notifier([&]() {
      for (int i = 0; i < 3; ++i) {
        ...
        signal.Reset(); // Notify current waiters and arm next cycle
      }
    });
    notifier.detach();
...
WPromise<void> MultipleWaitersExample() {
  return [] -> Promise<void> {
